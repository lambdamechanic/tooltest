...
fn collect_constraints_inner(
    ...
    if let JsonValue::Array(array) = value {
        ...
        if let Some(items_schema) = schema.get("items") {
            if let Some(items_object) = items_schema.as_object() {
                for (idx, item) in array.iter().enumerate() {
                    path.push(PathSegment::Index(idx));
                    collect_constraints_inner(items_object, root, item, path, constraints)?;
                    path.pop();
                }
            }
        }
    ...
    if let JsonValue::Object(map) = value {
        ...
        if let Some(JsonValue::Array(required)) = schema.get("required") {
            for required_name in required.iter().filter_map(JsonValue::as_str) {
                if !map.contains_key(required_name) {
                    constraints.push(Constraint {
                        path: nonempty_path(path),
                        kind: ConstraintKind::Required(required_name.to_string()),
                    });
                }
            }
        }
...
fn collect_violations_inner(
    ...
    if let Some(JsonValue::String(schema_type)) = schema.get("type") {
        match (schema_type.as_str(), value) {
    ...
    match value {
        ...
        JsonValue::Array(array) => {
            ...
            if let Some(JsonValue::Object(items_schema)) = schema.get("items") {
                for (idx, item) in array.iter().enumerate() {
                    path.push(PathSegment::Index(idx));
                    collect_violations_inner(items_schema, root, item, path, violations);
                    path.pop();
                }
            }
        }
