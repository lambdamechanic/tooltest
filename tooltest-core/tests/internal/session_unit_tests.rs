...
fn http_transport_config_strips_bearer_prefix() {
    let mut config = HttpConfig::new("https://example.com/mcp").expect("http config");
    config.auth_token = Some("Bearer test-token".to_string());
...
fn http_transport_config_preserves_raw_token() {
    let mut config = HttpConfig::new("https://example.com/mcp").expect("http config");
    config.auth_token = Some("raw-token".to_string());
    let transport_config = http_transport_config(&config);
...
fn http_transport_config_skips_missing_token() {
    let config = HttpConfig::new("https://example.com/mcp").expect("http config");
    let transport_config = http_transport_config(&config);
