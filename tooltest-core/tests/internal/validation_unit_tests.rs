...
async fn list_tools_helpers_report_errors_in_tests() {
    ...
    let _error = list_tools_http(&http, &SchemaConfig::default())
        .await
        .expect_err("http error");

    let stdio = StdioConfig::new("fail");
    let error = list_tools_stdio(&stdio, &SchemaConfig::default())
        .await
        .expect_err("stdio error");
...
#[test]
fn list_tools_error_formats_schema_error() {
    let error = ListToolsError::from(SchemaError::InvalidListTools("bad".to_string()));
    assert!(error.to_string().contains("schema error"));
}

#[test]
fn list_tools_error_formats_session_error() {
    let error = ListToolsError::from(SessionError::Transport(Box::new(std::io::Error::other(
        "transport",
    ))));
    assert!(error.to_string().contains("session error"));
}
...
#[test]
fn output_schema_validator_rejects_unknown_schema_version() {
    let tool = tool_with_output_schema(
        "noop",
        json!({
...
#[tokio::test]
async fn list_tools_session_reports_error() {
...
async fn list_tools_with_connector_uses_session() {
    let transport = TestTransport::new(vec![test_tool("echo")], []);
    let tools = list_tools_with_connector(
        ConnectorOutcome::Ok(transport),
...
async fn list_tools_with_connector_accepts_http_config() {
    ...
    let tools = list_tools_with_connector(ok_config, &SchemaConfig::default(), http_test_connector)
        .await
        .expect("tools");
    assert_eq!(tools.len(), 1);
