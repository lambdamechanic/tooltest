...
use rmcp::model::{
    ...
    ListPromptsRequest, ListResourcesRequest, ListToolsRequest, ListToolsResult, NumberOrString,
    PaginatedRequestParam, ReadResourceRequest, ReadResourceRequestParam, ServerJsonRpcMessage,
    ServerResult,
};
...
fn list_prompts_from_response(response: ServerJsonRpcMessage) -> Option<Vec<rmcp::model::Prompt>> {
    match response {
        ServerJsonRpcMessage::Response(response) => match response.result {
...
fn prompt_text_from_message_returns_none_for_non_text() {
    let resource = rmcp::model::RawResource::new("tooltest://example", "example").no_annotation();
...
async fn run_tooltest_call_tool_panic_is_internal_error() {
    ...
    fn panic_execute(_input: TooltestInput) -> TooltestExecuteFuture {
        Box::pin(async move {
            panic!("boom");
        })
    }

    let worker = TooltestWorker::new_with_config(TooltestWorkerConfig::default(), panic_execute)
        .expect("worker");
